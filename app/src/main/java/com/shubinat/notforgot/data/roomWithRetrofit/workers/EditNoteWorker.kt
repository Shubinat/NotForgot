...
class EditNoteWorker(private val context: Context, private val workerParameters: WorkerParameters) :
    ...
    private val model by lazy {
        val json = inputData.getString(NOTE_KEY)
        val gson = GsonBuilder().create()
        return@lazy gson.fromJson(json, NoteModel::class.java)
    }
    private val user by lazy {
        val userId = inputData.getInt(USER_ID_KEY, 0)
    ...
    override fun doWork(): Result {
        ...
        try {
            ...
            val result = api.putNote(token, model.id, model).execute()
            when (result.code()) {
    ...
    companion object {
        ...
        const val USER_ID_KEY = "USER_KEY"
